    ...
    for (const [nome, item] of Object.entries(groupedCart)) {
      ...
      const itemHTML = `
        <div class="flex flex-col md:flex-row justify-between items-center p-2 border-b border-gray-300 space-y-4 md:space-y-0">
          ...
          <!-- Botão de remover -->
          <button class="bg-red-600 text-white border-[2px] border-red-400 text-md font-semibold px-6 py-3 rounded-lg shadow-md transform hover:scale-105 hover:shadow-lg transition-all duration-300 hover:bg-red-600" onclick="removeFromCart('${nome}')">
            Remover 
          </button>
        </div>
      `;
